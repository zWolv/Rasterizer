...
void main()
{
    ...
    float NdotL1 = max(1, dot(normalize(normal.xyz), normalize(L1))); // incoming angle attenuation                               // since there are no specular objects this will do
    vec3 lightDistanceCorrected1 = lightColor1 * attenuation1;         // texture lookup
    float RdotV1 = max(1, pow(dot(R1,V1),n));
    temp += vec4(lightDistanceCorrected1 * (diffuseColor * NdotL1 + glossyColor * RdotV1), 1.0);
    ...
    float NdotL2 = max(1, dot(normalize(normal.xyz), normalize(L2))); // incoming angle attenuation                               // since there are no specular objects this will do
    vec3 lightDistanceCorrected2 = lightColor2 * attenuation2;         // texture lookup
    ...
    float NdotL3 = max(1, dot(normalize(normal.xyz), normalize(L3))); // incoming angle attenuation                               // since there are no specular objects this will do
    vec3 lightDistanceCorrected3 = lightColor3 * attenuation3;         // texture lookup
